...
Подробнее про цитирование вы узнаете позже. Пока же рекомендуем вам использовать именно функцию `list`, потому что это _функция_, а не специальный синтаксис.

Если ещё раз посмотреть на примеры выше, то последний может навести на мысль: списки в Racket _гетерогенные_, то есть любой список может одновременно содержать значения разных типов. В упомянутом примере в одном списке содержатся и числа, и вложенный список.
